...
describe('useNavihanDisplay', () => {
  ...
  it('defaults to all types visible when nothing is stored', async () => {
  ...
  it('reads a stored selection on init', async () => {
    ...
    const useNavihanDisplay = await freshUseNavihanDisplay();
    const { visible } = useNavihanDisplay();
    expect(visible).toEqual({ bm: false, flot: true, flotEst: true, flotObs: true, pm: false });
  });
  ...
  it('persists the selection to localStorage', async () => {
    ...
    expect(JSON.parse(localStorage.getItem(STORAGE_KEY)!)).toEqual({ bm: false, flot: true, flotEst: true, flotObs: true, pm: true });
  });
  ...
  it('completes a partial stored object with defaults (true)', async () => {
    ...
    const useNavihanDisplay = await freshUseNavihanDisplay();
    const { visible } = useNavihanDisplay();
    expect(visible).toEqual({ bm: false, flot: true, flotEst: true, flotObs: true, pm: true });
  });
